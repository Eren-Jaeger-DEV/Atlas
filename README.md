...
1. **Professional IDE Experience**: High-speed code editing, instant high-tech CSS dark splash skeleton (<10ms UI paint inside `index.html`), pre-compiled production build launcher (`pnpm start`), standalone Linux `.deb` installer & `.AppImage` packaging (`pnpm package:deb`), framer-motion spring animations, Web Worker LSP isolation, CallGraphVisualizer (interactive SVG node graph), PromptStudio (AI prompt engineering workbench with live execution), WorkflowEditor (interactive visual DAG workflow builder with live SVG node connectors & IPC file saving), MultiDiffTabViewer (multi-file batch diff review tab with live git status loading, per-file stage/unstage badges, and 1-click commit bar), ProjectHealthDashboard (tech debt meter), TrajectoryReplayViewer (timeline scrubber), ToastBannerManager (glassmorphic animated notifications), GitBlameGutter (inline line annotations), WorkspaceProfileSwitcher, live Mermaid.js chat diagram renderer, Jupyter Notebook viewer (`.ipynb`), **Atlas Canvas In-Editor Reactive Notebook Engine (`ReactiveNotebookEngine.ts` & `AtlasCanvasPanel.tsx`)** (reactive cell execution DAG with inline JSON/SVG chart renderers for `.atlas-nb` documents), PluginViewerPane (live sanitized HTML & markdown viewer plugin integration), SettingsConfigViewer (graphical settings manager with dedicated **Keybindings** & **Plugins & Permissions** categories and **Reset Category Defaults**), CommandPaletteQuickPicker (`Ctrl+Shift+P`), **Interactive Keyboard Shortcuts Cheat Sheet Modal (`Ctrl+/`)**, **Monaco Minimap Code Radar Overlay** (overview ruler Git change status bands), **Real-Time Git Explorer Status & Badging Engine** (`U`, `M`, `A`, `D`, `R`, `C` letter badges, status text colors, and parent folder status dots `•`), **Atlas Horizon Workbench UI (`HorizonPanel.tsx`)** (interactive spec-driven stage pipeline HUD bar & parallel swarm wave matrix), TerminalSuggestEngine (1-click AI terminal fixes), rich `@mentions` chat composer, **Universal Glassmorphic Context Menus** across File Explorer, Editor Tabs, Terminal Canvas, and Source Control Panel (`useContextMenu`), in-app `DialogProvider` modal confirmations, animated toast stack, interactive status bar quick-picks (`StatusBarRegistry`), floating inline AI `Ctrl+K` bar, tabs, split panes, interactive SVG dependency graphs, 3-way merge conflict editor, WebPreviewPanel (integrated dev server live web app preview tab with Desktop/Tablet/Mobile viewports), Side-by-Side Dual Pane Diff Split (`ComposerDiff`), Full Voice Input Mode (browser SpeechRecognition hands-free prompt dictation), **Structural AST Pattern Search Engine** (`AstSearchPanel` — wildcard `$VAR` syntax search across workspace source files with 1-click structural replacement, 4 built-in presets, and named binding extraction), **Local Model Radar** (`LocalModelRadarPanel` — zero-config discovery of Ollama/LM Studio/vLLM/Llama.cpp/GPT4All/Jan with parallel health probing and 1-click model selection), **Impact Radar** (`ImpactRadarPanel` — downstream blast radius visualizer with hop-distance color chips, risk level banners, and API surface grouping), **Shadow Worktree Verify** (`ShadowWorktreePanel` — isolated git worktree sandbox for AI change verification with expandable command result cards), and integrated terminal.
2. **Deterministic Project Intelligence**: Fast AST symbol indexing, WorkspaceSearchIndexer (fast regex & glob text search), SecurityAuditEngine (AST secret & vulnerability scanner), Shadow Workspace AST Diagnostics Engine (`verifyAST` health check badges & 1-click rollback toasts), **Atlas Cortex GraphRAG Knowledge Graph (`GraphRagEngine.ts` & `AtlasCortexPanel.tsx`)** (semantic code relationship indexer mapping callers, callees, and dependencies in a 2-hop neighborhood to augment AI prompts with 0% hallucinated calls), **Atlas Torch CPU & Heap Flamegraph Profiler (`FlamegraphProfiler.ts` & `AtlasTorchPanel.tsx`)** (one-click execution profiler computing exclusive self-time vs total time, rendering stacked call frame trees, and isolating >20% execution hotspots with 1-click AI optimization prompts), **Atlas Chronicle AI Commit Narrator (`CommitNarrator.ts` & `AtlasChroniclePanel.tsx`)** (semantic git history narrator evaluating commit risk levels LOW/MED/HIGH, extracting impacted symbols, and auto-drafting formatted Conventional Commit messages), **Atlas Crucible Mutation Score Visualizer (`MutationTestEngine.ts` & `AtlasCruciblePanel.tsx`)** (live background mutation testing engine calculating true test effectiveness score via AST fault injection with 1-click AI test generation for surviving mutants), **Atlas Lens Trigram Search Indexer (`AtlasLens.ts` & `AtlasLensPanel.tsx`)** (persistent 3-character trigram index of all workspace files for sub-50ms regex/substring search across 100,000+ files without linear process spawning), **Atlas Sentinel AST Security Radar (`LiveSecurityScanner.ts` & `AtlasSentinelPanel.tsx`)** (real-time always-on AST vulnerability & secret scanner running on file save — detects hardcoded credentials CWE-798, SQL injection CWE-89, command injection CWE-78, XSS CWE-79, and eval risks with 1-click remediation guidance), **Atlas Prism Structural AST Diff Engine (`AtlasPrism.ts` & `AtlasPrismDiffPanel.tsx`)** (AST-aware semantic git diff visualizer with hunk classification for function signatures, logic changes, import mods, and formatting-only collapse filters), reverse-engineered benchmark parity (Cursor agent-exec/retrieval, Antigravity workflow/rule editor, VS Code extension host), CompetitorSettingsImporter (Cursor/Windsurf migration), FeatureFlagManager (Statsig flags & telemetry), cycle detection, project health dashboard, and definition peek popovers.
